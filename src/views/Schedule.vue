<template>
  <main
    v-if="$store.getters['auth/user'] && $store.getters['planner/schedules']"
  >
      ...
      <div class="calendar" v-if="schedule">
        <div class="colDays">
          <div class="row" v-for="(day, index) in 7" :key="index">
            <span class="dayName">
              {{ $store.getters["date/dayNames"][index] }}</span
            >
            <span class="date">{{
        ...
        <div class="colCalendar" ref="calendar">
          ...
          <div class="row" v-for="(day, index) in schedule" :key="index">
            <div
              ...
              :style="{
                width: `${
                  timeRangeToPercentage(day.start, day.end).percentage
                }%`,
                left: `${
                  timeRangeToPercentage(day.start, day.end).startPoint
                }%`,
              }"
            ...
            >
              ...
              <span class="time"
                >{{ formatTime(day.start) }} - {{ formatTime(day.end) }}</span
              >
              <span
    ...
    >
      <template v-slot:main>
        ...
        <div class="shift-info-group">
          ...
          <span class="value"
            >{{ formatTime(activeShift.start) }} -
            {{ formatTime(activeShift.end) }}</span
          >
        </div>
        <div class="shift-info-group">
...
export default {
  ...
  computed: {
    ...
    showRange() {
      const schedule =
        this.$store.getters["planner/schedules"][
          this.$store.getters["date/weekId"]
        ];
  ...
  methods: {
    ...
    setActiveShift(shift, index) {
      ...
      // Construct object
      this.activeShift = {
        date: this.$store.getters["date/dates"][index].toLocaleDateString(
